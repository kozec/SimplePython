...
public class PythonCompiler {
	...
	public UserFunctionObject doCompile(String_inputContext sctx, List<InternalDict> globals, 
			List<String> args, String vargarg, InternalDict defaults, InternalDict locals) {
	...
	private void compileTry(Try_stmtContext try_stmt, List<PythonBytecode> bytecode, ControllStack cs) {
		...
		if (try_stmt.try_except().size() > 0) {
			...
			if (try_stmt.else_block() != null)
				...
				elseJump = addBytecode(bytecode, Bytecode.JUMPIFNONE, try_stmt.start);
			...
			for (Try_exceptContext ex : try_stmt.try_except()) {
				if (ex.except_clause().test().size() == 0) {
					...
					addBytecode(bytecode, Bytecode.ISINSTANCE, try_stmt.start);
					cb = addBytecode(bytecode, Bytecode.JUMPIFTRUE, try_stmt.start);
					exceptJumps.add(cb);
				}
			}
			if (!alwaysHandled) {
	...
	private void compileInclude(IncludeContext include, List<PythonBytecode> bytecode, final ModuleData m) {
		...
		if (m.getResolver() == null)
			throw new me.enerccio.sp.errors.Error("can't use include in this module");
		final ModuleData toInclude = PythonRuntime.runtime.resolveModule(filename, new StringObject(m.getPackageResolve(), true));
		pythonParser parser;
		try {
			parser = StaticTools.ParserGenerator.parse(toInclude);
		} catch (Exception e) {
			throw new SyntaxError("failed to parse source code of " + filename, e);
		}
		ModuleData prevModule = module;
		module = new ModuleData() {
			// filename here
			@Override public ModuleResolver getResolver() { 
				return toInclude.getResolver();
			}
			...
			@Override public String getFileName() {
				return toInclude.getFileName();
			}
			@Override
			public String getPackageResolve() {
				return toInclude.getPackageResolve();
			}
			@Override
	...
	private void compileSwitch(Switch_stmtContext ctx, List<PythonBytecode> bytecode, ControllStack cs) {
		...
		for (int i=0; i<ctx.case_block().size(); i++) {
			...
			putGetAttr(NumberObject.__EQ__, bytecode, ctx.case_block(i).test().start);
	...
	private void compileFunction(FuncdefContext funcdef,
		...
		Utils.putPublic(fnc, "__location__", new StringObject(module.getName() + " line " + funcdef.start.getLine() + ", char " + funcdef.start.getCharPositionInLine()));
	...
	private void doCompileFunction(ParseTree suite, List<PythonBytecode> bytecode, Token t) {
		stack.push();
...
//			compileParentheseslessCall(smstmt.parenthesesless_call(), bytecode);
	...
	private void compileDel(AtomContext atom, List<PythonBytecode> bytecode, boolean delkeyOnly) {
		if (!delkeyOnly){
			if (atom.nname() != null){
	...
	private void compile(Print_stmtContext ctx, List<PythonBytecode> bytecode) {
		if (ctx.push() != null) {
			...
			if (eol){
				cb = addBytecode(bytecode, Bytecode.LOADGLOBAL, ctx.stop);
	...
	private void compile(Expr_stmtContext expr, List<PythonBytecode> bytecode) {
		if (expr.augassignexp() != null) {
			...
			for (ParseTree pt : rest){
				...
				addBytecode(bytecode, Bytecode.DUP, tc == null ? yt.start : tc.start);
	...
	private void compile(ComparisonContext ctx, List<PythonBytecode> bytecode) {
		if (ctx.getChildCount() > 1){
			if (ctx.getChild(1).getText().equals("in") || ctx.getChild(1).getText().equals("notin")){
				...
				cb = addBytecode(bytecode, Bytecode.CALL, ((ExprContext) ctx.getChild(2)).start);
				cb.intValue = 1;
