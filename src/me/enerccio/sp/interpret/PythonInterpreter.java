...
//		System.out.println(CompiledBlockObject.dis(frame));
	...
	private void unpackKwargs(FrameObject o, Stack<PythonObject> stack) {
		...
		PythonObject iterator = Utils.run("iter", execute(true, keysFn, null)).get(GeneratorObject.NEXT);
		if (o.kwargs == null)
			o.kwargs = new KwArgs.HashMapKWArgs();
		try {
			while (true) {
			...
			if (target == null){
				...
				if (target == null || !(target instanceof ModuleObject)){
					...
					synchronized (PythonRuntime.runtime){
						...
						if (target == null)
							target = PythonRuntime.runtime.getModule(mm, resolvePath == null ? null : new StringObject(resolvePath, true), injectGlobals);
					}
				}
			} else {
				...
				if (target.get(mm, null) != null) {
					...
					target = PythonRuntime.runtime.getModule(mm, new StringObject(((ModuleObject)target).getModuleData().getPackageResolve(), true), injectGlobals);
				}
			}
			pythonImport(environment, variable, modulePath, target, injectGlobals);
		}
	}
